...
- ✅ **Daily Log Injection + Israel-local Timestamps** *(completed 2026-04-17, PR #23)*: Added `daily_log_today: list[dict]` field to `ContextSchema`. Bot fetches today's log fresh per-message (not cached, since the log mutates on every commit) and sends it in the `context` HTTP body. `response_node` renders an always-present `## Today's Log` section in the SystemMessage (with explicit "Nothing logged yet today" when empty — useful signal for empty-log coaching). Folded in Bug 2 fix: `_serialize_log` in `src/services/daily_log_service.py` now converts UTC timestamps to Israel local time before `.isoformat()`, fixing UTC display leaks across the entire stack (`stats_node`, `query_food_logs` tool, and the new injection path). Also hardcoded `USER_TIMEZONE = ZoneInfo("Asia/Jerusalem")` in `src/config.py` for the time injection in `response_node`. Resolves bot UX audit findings F1, F3 T1/T2/T3, F3 T4/T5, F6, and partial F2. Plan: `docs/plans/daily-log-injection-and-israel-tz-serialization.md`.
...
- **Real-Time Nutrition Plan Refresh**: When the coach updates a trainee's nutrition plan via `set_plan.py`, the bot's in-memory session cache doesn't reflect the change until the session expires (30 min) or the bot restarts. Implement a lightweight refresh mechanism — e.g., a `/refresh` bot command that re-loads the profile from DB, or a webhook/event that the script triggers after updating. Avoid reloading the profile from DB on every single message (unnecessary overhead for a rarely-changing field).
- **Nutrition Plan History**: Move nutrition plans from the `user_profiles.nutrition_plan` text column into a dedicated `nutrition_plans` table with versioning. Enables plan history (coach changes a trainee's targets over time), date ranges (`active_from` / `active_until`), and structured macro target columns alongside the free-text plan for programmatic queries (e.g., coach dashboard showing compliance percentages). The POC stores plans as a single text field on `user_profiles` — this item upgrades that to a first-class entity.
...
- **Per-User Language Preference**: Store a `language` column on `UserProfile` (default `"en"`, allowed values `"en"` / `"he"`). Modify the i18n loader to accept a runtime `lang` parameter alongside the global env-var default, and update the bot/graph to pass each user's stored preference. Enables coaches running a single bot instance with a mix of English- and Hebrew-speaking trainees. Pre-requisite: i18n message extraction (v2 of `docs/plans/i18n-message-extraction.md`).
- **Routing Style Audit — `Command` vs `add_conditional_edges`**: FitPal currently uses `add_conditional_edges` for all routing (`route_parser`, `route_after_selection`, `route_after_calculate_macros`) except `confirmation_node`, which uses `Command(goto=...)` because routing is computed inside an interrupt loop. The boundary is convention-driven, not principled — all three conditional-edge sites *could* be migrated to `Command`-returning nodes with no behavior change. Tradeoffs to evaluate: (1) **Conditional edges** keep graph topology visible in `nutritionist.py` and in LangSmith Studio's visual graph, allow shared route functions, and work well when routing is a short function over state fields the node was already going to write. (2) **`Command`** colocates routing decisions with the code that produced them, removes the need for redundant route functions, gives type safety via `Command[Literal[...]]`, and enables dynamic targets (subgraphs, parent graphs). The interesting question is whether visibility-in-the-graph-file is worth the redundancy of separate route functions, or whether colocating decisions with their producing nodes is cleaner long-term. Worth a short spike: rewrite one node (probably `input_parser`) both ways and compare readability/debuggability before deciding on a project-wide convention. No urgency — current setup works fine.
- **HITL Confirmation: Add-Item Edit Type**: The `ItemEdit` schema in `src/schemas/confirmation_schema.py` currently supports two `edit_type` values: `change_amount` and `remove`. There is no `add` operation. As a result, a user reply that mixes confirmation with new items (e.g. "yes and 2 slices of bread") will most likely be classified as a plain `confirm` by the LLM and the new items will be silently dropped — the user gets the original batch logged with no indication that their addition was ignored. Less likely but worse: the LLM may misinterpret the addition as a `change_amount` on an existing item and corrupt its macros. Fix requires: (1) extending `ItemEdit.edit_type` literal with `"add"` plus a `new_food_name` field, (2) updating `_apply_edits` in `confirmation_node.py` to re-enter the calculate-macros pipeline (DB lookup or LLM estimation) for new items — structurally different from the in-place mutations that current edits do, (3) updating `prompts/confirmation_parser.md` with examples, (4) tests for the new path. Until built, the workaround is to confirm the current batch and then send the new items as a separate message.
