...
- **Human-in-the-Loop (HITL)**: Graph routing supports pausing execution (e.g., routing to Response Node on `ESTIMATED` status) to ask the user for confirmation, then resuming safely downstream (e.g., straight to Calculation via `CONFIRM_ESTIMATION` intent).

## 6. Agent Operational Constraints
- **NO PROACTIVE COMMITS**: The agent must NEVER proactively run `git add`, `git commit`, or `git push` in the middle of a workflow or task execution. All Git operations MUST be strictly reserved for when the user explicitly triggers the `[/commit]` workflow. Ensure code is validated and the user determines when the task is fully "complete" before attempting to interact with Git tracking.
