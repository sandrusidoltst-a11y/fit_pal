...
- **Action**: Create Pydantic models.
    ...
    - `FoodIntakeEvent`: 
        - `action`: Literal["log_food", "query_food", "chitchat"]
...
- **Action**:
    ...
    - Function `input_parser_node(state: AgentState)`:
        ...
        - Invoke LLM.
        - Return `{"pending_food_items": result.items}`.
...
- **Action**:
    ...
    - Add Conditional Edge:
        - If `action == "log_food"`, go to `food_lookup` (future node).
        - If `action == "chitchat"`, go to `response` (end).
...
Create a temporary test script `tests/debug_parser.py` that mocks the State and calls `input_parser_node`.
